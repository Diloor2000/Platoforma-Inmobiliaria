...
export default async function DashboardPage() {
  const [
    ...
    supabase.from('profiles').select('id, full_name, email, role, status').order('created_at', { ascending: false }).limit(200),
    supabase.from('appointments').select('id, property_id, user_id, appointment_date, appointment_time, notes, created_at').order('appointment_date', { ascending: false }).limit(100),
  ]);
  ...
  const appointments: Array<{ id: string; property_id: string; user_id: string; appointment_date: string; appointment_time: string; notes?: string | null; created_at?: string }> = appointmentsData ?? [];
    ...
    <main className="min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
        ...
        <div id="citas" className="mb-8 scroll-mt-24">
            ...
            <div className="space-y-2 rounded-2xl border border-slate-200/80 bg-white/80 p-4 shadow-lg backdrop-blur-xl dark:border-slate-700/80 dark:bg-slate-900/80">
              {appointmentsWithDetails.map((apt) => (
                <div
                  key={apt.id}
                  className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-100 bg-slate-50/50 py-3 px-4 dark:border-slate-800 dark:bg-slate-800/50"
                >
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-slate-200 text-left text-sm dark:divide-slate-700">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  ...
                  {profiles.map((p) => (
                    <ProfileRow key={p.id} profile={p as Profile & { id: string; email?: string; full_name?: string; role: string; status: string }} />
                  ))}
                </tbody>
              </table>
            </div>
